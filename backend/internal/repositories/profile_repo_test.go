	...
	tsc := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, repo *ProfileRepository, mock sqlmock.Sqlmock) {
				...
				p, err := repo.CreateProfile(context.Background(), p)
	...
	tsc := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, repo *ProfileRepository, mock sqlmock.Sqlmock) {
				...
				mock.ExpectExec("UPDATE links SET url = ? WHERE id = ?").WillReturnResult(sqlmock.NewResult(1, 1))
				mock.ExpectCommit()

				p, err := repo.UpdateProfile(context.Background(), p)
				require.NoError(t, err)
				require.Equal(t, int(1), p.ID)
		...
		},
		{
			...
			test: func(t *testing.T, repo *ProfileRepository, mock sqlmock.Sqlmock) {
				mock.ExpectBegin()
				mock.ExpectExec("UPDATE profiles SET first_name = ?, last_name = ?, email = ?, avatar_url = ? WHERE id = ?").WillReturnResult(sqlmock.NewResult(1, 1))
				mock.ExpectExec("UPDATE links SET url = ? WHERE id = ?").WillReturnError(errors.New("failed to update links"))
				mock.ExpectRollback()
